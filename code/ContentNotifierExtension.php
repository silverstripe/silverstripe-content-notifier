...
class ContentNotifierExtension extends DataExtension
{
    ...
    protected function resolve($approved) {
        ...
        if($this->getSetting('delete_on_resolve')) {
            if($object = $this->getQueue()) $object->delete();
        }
    }
    ...
    public function EmailSummary() {
        $template = $this->getSetting('email_notifier_template') ?: Config::inst()->get('ContentNotifierEmail','item_template');
    ...
    public function getStatus() {
        return $this->owner->ContentNotifierApproved ? _t('ContentNotifier.APPROVED','APPROVED') : _t('ContentNotifier.UNAPPROVED','UNAPPROVED');
    }
